...
ViberBot.prototype.sendMessage = function(optionalUserProfile, messages, optionalTrackingData, optionalChatId, optionalSender) {
	return this._sendMessages(optionalUserProfile, messages, REQUEST_TYPE.SEND_MESSAGE, optionalTrackingData, optionalChatId, optionalSender);
};
...
ViberBot.prototype._sendMessages = function(userProfile, messages, requestType, optionalTrackingData, optionalChatId, optionalSender) {
	...
	_.each(messages, message => {
		...
		promise = promise.then(() => new Promise((resolve, reject) => {
			if (requestType == REQUEST_TYPE.SEND_MESSAGE) return self._sendMessageFromClient(userProfile, message, null, null, optionalChatId, message.minApiVersion, optionalSender).then(response => resolveCallback(response, message, resolve, reject));
	...
	return promise.then(() => new Promise((resolve, reject) => {
		if (requestType == REQUEST_TYPE.SEND_MESSAGE) {
			return self._sendMessageFromClient(userProfile, lastMessage, optionalTrackingData, lastMessage.keyboard, optionalChatId, lastMessage.minApiVersion, optionalSender)
...
ViberBot.prototype._sendMessageFromClient = function(optionalUserProfile, message, optionalTrackingData, optionalKeyboard, optionalChatId, optionalMinApiVersion, optionalSender) {
	const jsonMessage = message.toJson();
	let receiver = null;
	if (optionalUserProfile) {
		receiver = optionalUserProfile.id;
	}
	return this._client.sendMessage(receiver, jsonMessage.type, jsonMessage, optionalTrackingData, optionalKeyboard, optionalChatId, optionalMinApiVersion, optionalSender);
};
